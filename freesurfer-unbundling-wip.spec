...
# Keep only: dicom, dcm2niix, cephes, svm (no system equivalent)
sed -i '/^  glut$/d;/^  jpeg$/d;/^  expat$/d;/^  xml2$/d' packages/CMakeLists.txt
sed -i '/^  minc$/d;/^  netcdf$/d;/^  nifti$/d;/^  gifti$/d' packages/CMakeLists.txt
sed -i '/^  tetgen$/d;/^  nrrdio$/d' packages/CMakeLists.txt

# Replace bundled include paths with system paths in utils/CMakeLists.txt
sed -i 's|${CMAKE_SOURCE_DIR}/packages/jpeg|/usr/include|' utils/CMakeLists.txt
sed -i 's|${CMAKE_SOURCE_DIR}/packages/xml2|/usr/include/libxml2|' utils/CMakeLists.txt
sed -i 's|${CMAKE_SOURCE_DIR}/packages/minc|/usr/include/libminc|' utils/CMakeLists.txt
sed -i 's|${CMAKE_SOURCE_DIR}/packages/netcdf|/usr/include|' utils/CMakeLists.txt
sed -i 's|${CMAKE_SOURCE_DIR}/packages/nifti|/usr/include/nifti|' utils/CMakeLists.txt
sed -i 's|${CMAKE_SOURCE_DIR}/packages/gifti|/usr/include/gifti|' utils/CMakeLists.txt
sed -i 's|${CMAKE_SOURCE_DIR}/packages/nrrdio|/usr/include/teem|' utils/CMakeLists.txt

# Replace bundled library target names with system library names in utils link
# Note: jpeg→jpeg, xml2→xml2, minc→minc2, netcdf→netcdf, nifti→niftiio+znz,
#       gifti→giftiio, nrrdio→teem, tetgen→tet, expat→expat
...
# Fix nifti references: system libs are niftiio + znz
find . -name CMakeLists.txt -not -path './packages/*' -exec \
    sed -i '/target_link_libraries/s/\bnifti\b/niftiio znz/g' {} +
find . -name CMakeLists.txt -not -path './packages/*' -exec \
    sed -i 's|${CMAKE_SOURCE_DIR}/packages/nifti|/usr/include/nifti|g' {} +

# FreeSurfer includes "NrrdIO.h" but teem provides "nrrd.h" — create a wrapper
cat > include/NrrdIO.h << 'NRRDEOF'
/* Wrapper for system teem nrrd.h — replaces bundled NrrdIO standalone */
#include <teem/nrrd.h>
#include <teem/biff.h>
#include <teem/air.h>
NRRDEOF
